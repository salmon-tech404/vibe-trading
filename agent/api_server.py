...
# --- Binance USD-M Futures Live Execution ---
from src.api.futures_routes import register_futures_routes  # noqa: E402
register_futures_routes(app)

# --- Physical Reflections & Strategy Active Learning Loop ---
from src.api.reflection_routes import register_reflection_routes  # noqa: E402
register_reflection_routes(app)

# --- In-Flight Position Evaluation & LLM Circuit Breaker ---
from src.api.position_eval_routes import register_position_eval_routes  # noqa: E402
register_position_eval_routes(app)

# --- Autonomous Closed-Loop Self-Optimizing Engine & Changelog ---
from src.api.optimization_bridge import register_optimization_bridge_routes  # noqa: E402
register_optimization_bridge_routes(app)

from pydantic import BaseModel as _BaseModel
class _TerminalLogRequest(_BaseModel):
    level: str = "INFO"
    category: str = "BOT"
    message: str

@app.post("/api/terminal-log")
async def _handle_terminal_log(payload: _TerminalLogRequest):
    """Receive live log streams from Frontend Bot and print directly to the Terminal."""
    from datetime import datetime as _dt
    now = _dt.now().strftime("%H:%M:%S")
    color = "\033[92m" if payload.level == "TRADE" else "\033[93m" if payload.level == "WARN" else "\033[91m" if payload.level == "ERROR" else "\033[96m"
    reset = "\033[0m"
    print(f"[{now}] {color}[{payload.level}] [{payload.category}]{reset} {payload.message}", flush=True)
    return {"status": "ok"}
